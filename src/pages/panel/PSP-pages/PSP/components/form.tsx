...
import { PSPInputs } from "../core/_model";
...
export const Form = ({
  ...
  const { submitCreatePSP, isPending: createPending } = useCreatePSP({
    onSuccess: () => {
      Toast("success");
      onCloseHandler();
  ...
  const { data: editData, isPending: fetchPending } = useGetPSPDetails(
    editId ? editId : 0,
    !!editId
  );
  ...
  const onSubmit: SubmitHandler<PSPInputs> = (data) => {
    editId ? submitUpdatePSP(data) : submitCreatePSP(data);
  };
  const closeHandler = () => {
    resetEditId && resetEditId(null);
    onCloseHandler();
